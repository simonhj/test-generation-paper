...
\begin{figure}[t]
\centering
\includegraphics[trim=47 378 380 42,clip,width=\columnwidth]{figs/appModel.pdf}
\vspace*{-15pt}
...
The price of an item must be greater &
$({\tt price} > 0) \Longrightarrow ({\tt item.price} = {\tt price})$ \\
Item & & & & than zero & \\
\hline
...
\{{\tt Customer}\} &
The credit limit can be incremented for &
$({\tt cust.balType} = {\tt Credit} \wedge {\tt amount} > 0) \Longrightarrow$ \\
