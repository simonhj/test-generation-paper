...
a few operations that modify entities. Instead, the search space can become
exponential, if there exist many operations that modify each entity. To
address this issue, we use an optimization based on unsatisfied core 
that helps prune the search space, thereby handle large models.

\paragraph*{Extract Unsatisfiable Core} In particular, if the composed formula in
Line 9 is unsatisfiable, we extract the unsatisfiable core, \subject{ucore},
...
that create or modify \subject{Order}, and identifies the candidate operation
...
After creating the new concrete sequence, we checks satisifiability of the new
...
algorithm checks the following two aspects for each sequence $cseq$.
