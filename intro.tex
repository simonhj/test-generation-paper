...
\begin{alltt}
 ...
 CreateCustomer(\emph{State} st, \emph{BalanceType} bt, out: \emph{Customer} cust);
 AddCreditLimit(\emph{Customer} cust, int crLimit, out: \emph{Customer} cust1);
 CreateOrder(\emph{Customer} cust, out: \emph{Order} ord);
 CreateItem(\emph{int} price, out: \emph{Item} item);
 AddItemToOrder(\emph{Order} ord, \emph{Item} item, out: \emph{Order} ord1);
 GenerateInvoice(\emph{Order} ord1, out: \emph{Invoice} inv);  
\end{alltt}}%
\vspace*{-5pt}
